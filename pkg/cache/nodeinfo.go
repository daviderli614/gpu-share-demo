...
func (n *NodeInfo) Allocate(clientset *kubernetes.Clientset, pod *v1.Pod) (err error) {
	...
	// 2. Bind the pod to the node 当调度器找到满足条件的节点，调用bind方法
	if err == nil {
		binding := &v1.Binding{
...
func (n *NodeInfo) getAvailableGPUs() (availableGPUs map[int]uint) {
	allGPUs := n.getAllGPUs()  // 获取节点上的GPU
	usedGPUs := n.getUsedGPUs() //获取用户申请的GPU
	unhealthyGPUs := n.getUnhealthyGPUs() //获取不可用的GPU
	availableGPUs = map[int]uint{}  //可用GPU
